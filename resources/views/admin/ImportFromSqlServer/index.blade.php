...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
         ...
         <form action="{{ route('admin.sql.server.DataTransfer') }}" method="post" class="add_form" no-reset="true">
         {{csrf_field()}} 
            <div class="row">
                <div class="col-md-6">
                    <div class="card card-gray">
                        ...
                        <select class="form-control" name="from_district" id="from_district_select_box" select-triger="from_block_select_box" onchange="callAjax(this,'{{ route('admin.sql.server.districtWiseBlock') }}','from_block_select_box')">
                        <option selected disabled>Select From District</option>
                        @foreach ($from_districts as $from_district)
                        <option value="{{$from_district->DisttCode}}">{{$from_district->DisttCode}}-{{$from_district->DisttName}}</option>
                        @endforeach 
                        </select> 
                        </div>
                        <div class="form-group">
                        <label for="exampleInputEmail1">From Block</label> 
                        <select name="from_block" class="form-control select2" id="from_block_select_box" onchange="callAjax(this,'{{ route('admin.sql.server.BlockWiseWillage') }}'+'?id='+this.value+'&district_id='+$('#from_district_select_box').val(),'from_village_select_box')">
