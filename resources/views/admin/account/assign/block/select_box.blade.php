        ...
        <table class="table  table-bordered table-striped" id="class_section_list">
            ...
            <tbody>
                @foreach ($DistrictBlockAssigns as $DistrictBlockAssign)
                    <tr>
                        <td>{{ $DistrictBlockAssign->name_e}}</td> 
                        <td>
                            <a title="Delete" class="btn btn-xs btn-danger" select-triger="user_id" onclick="if (confirm('Are you Sure To Delete This Record')){callAjax(this,'{{ route('admin.Master.DistrictBlockAssignDelete',Crypt::encrypt($DistrictBlockAssign->id)) }}') } else{console_Log('cancel') }"  ><i class="fa fa-trash"></i></a>
                        </td> 
                    </tr> 
                @endforeach
            </tbody>
