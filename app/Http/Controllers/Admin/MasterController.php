...
// //-------districts--------------districts--------------districts---------------districts----//
  ...
  public function DistrictsTable(Request $request){
    try {
      $state_id = $request->id;
      $Districts = DB::select(DB::raw("select * from `districts` where `state_id` = $state_id order by `name_e`;"));
  ...
  public function districtsStore(Request $request,$id=null){  
    ...
    if (!empty($id)) {
      $rs_update = DB::select(DB::raw("update `districts` set `state_id` = $state_id, `code` = '$code', `name_e` = '$name_e', `name_l` = '$name_l' where `id` = $id;"));
      $response=['status'=>1,'msg'=>'Record Updated Successfully'];
    }else {
      $zpward = $request->zp_ward;
      $user=Auth::guard('admin')->user(); 
      $zpWard = DB::select(DB::raw("call `up_save_district` ($user->id, $state_id, '$code', '$name_e', '$name_l', $zpward);"));
  ...
  public function DistrictsZpWard($districts_id){
    try{
      $DistrictName = DB::select(DB::raw("select * from `districts` where `id` = $districts_id;")); 
  ...
  public function districtsDelete($id){
    try{
      $decrp_id = Crypt::decrypt($id);
      $rs_delete = DB::select(DB::raw("delete from `districts` where `id` = $decrp_id;"));
...
//     //------------block-mcs----------------------------//
  ...
  public function BlockMCSTable(Request $request){  
    try {
      ...
      $BlocksMcs = DB::select(DB::raw("select `bl`.`id`, `bl`.`code`, `bl`.`name_e`, `bl`.`name_l`, `blt`.`block_mc_type_e`, (select Count(*) from `ward_ps` where `blocks_id` = `bl`.`id`) as `pscount` from `blocks_mcs` `bl` inner join `block_mc_type` `blt` on `blt`.`id` = `bl`.`block_mc_type_id` where `bl`.`districts_id` = $d_id order by `bl`.`name_e`;"));
  ...
  public function BlockMCSStore(Request $request,$id=null){   
    ...
    $b_type_id = $request->block_mc_type_id;
    $bcode = str_replace('\'', '', trim($request->code));
    $name_e = str_replace('\'', '', trim($request->name_english)); 
    $name_l = str_replace('\'', '', trim($request->name_local_language)); 
    $stamp1 = str_replace('\'', '', trim($request->stamp_l1));
    $stamp2 = str_replace('\'', '', trim($request->stamp_l2));
    
    $admin = Auth::guard('admin')->user();
    $userid = $admin->id;
    if (empty($id)) { 
      $pswards = $request->ps_ward;
      $block_id = 0;  
    }else{
      $pswards = 0;
      $block_id = $id;
    }
    $rs_update = DB::select(DB::raw("call `up_save_block` ($block_id, $userid, $s_id, $d_id, '$bcode', '$name_e', '$name_l', $pswards, $b_type_id, '$stamp1', '$stamp2')"));
  ...
  public function BlockMCSEdit($id){
    try {
      $BlockMCTypes = DB::select(DB::raw("select * from `block_mc_type` order by `block_mc_type_e`;"));
      $BlocksMcs = DB::select(DB::raw("select * from `blocks_mcs` where `id` = $id"));
      return view('admin.master.block.edit',compact('BlocksMcs','BlockMCTypes'));
    } catch (Exception $e) {}
  }
  ...
  {
    try{
      $block_id = Crypt::decrypt($id);
      $rs_delete = DB::select(DB::raw("delete from `blocks_mcs` where `id` = $block_id"));
      return redirect()->back()->with(['message'=>'Block Deleted Successfully','class'=>'success']);
    } catch (Exception $e) {}  
  }
