...
{ 
  ...
  {   
    ...
    \Artisan::queue('preparevoterslip:generate',['district_id'=>$request->district,'block_id'=>$request->block,'village_id'=>$request->village,'ward_id'=>$request->ward,'booth_id'=>$request->booth,'slip_per_page'=>$request->slip_per_page]);
  ...
  public function PrepareVoterSlipDownload( )
  {
    ...
    return view('admin.master.PrepareVoterSlip.download',compact('Districts'));    
  }
  ...
  {  
    $VoterSlipProcessed = DB::select(DB::raw("select * from `voter_slip_processed` where `id` = $id limit 1;")); 
