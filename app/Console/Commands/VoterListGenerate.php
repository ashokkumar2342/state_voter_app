...
class VoterListGenerate extends Command
{
    ...
    { 
        ...
        $rs_VoterListProcessed = DB::select(DB::raw("select * from `voter_list_processeds` where `village_id` = $village_id and `ward_id` = $ward_id and `voter_list_master_id` = $voterListMaster->id and `booth_id` = $booth_id order by list_no ;"));

        foreach ($rs_VoterListProcessed as $key => $VoterListProcessed) {
            $rs_update = DB::select(DB::raw("update `voter_list_processeds` set `status` = 2, `start_time` = now() where `id` = $VoterListProcessed->id limit 1;"));
            ...
            $newId=DB::select(DB::raw("Update `voter_list_processeds` set `status` = 1, `finish_time` = now() where `id` = $VoterListProcessed->id;"));
